...
type ConversationNotesPanelProps = {
  conversation: Conversation
}

type NoteType = ClientNoteSummary["type"]
...
export function ConversationNotesPanel({
  ...
  const startEdit = (note: ClientNoteSummary) => {
    setMode("edit")
  ...
  const save = async () => {
    ...
    if (mode === "create") {
      await addConversationNote(conversation.id, {
        type,
        text: normalizedText,
        pinned,
      })
    ...
    if (mode === "edit" && editingNoteId) {
      await updateConversationNote(conversation.id, editingNoteId, {
        type,
        text: normalizedText,
        pinned,
      })
      ...
      {mode !== "idle" ? (
        <div className="mb-3 rounded-md border border-[var(--cf-border)] bg-[var(--cf-element)] p-2">
          ...
          <Textarea
            ...
            className="min-h-[90px] resize-none border-[var(--cf-border)] bg-[var(--cf-bg)] text-[12px] text-[var(--cf-text)] placeholder:text-[var(--cf-text-muted)]"
          />
              ...
              <div className="mb-2 flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void deleteConversationNote(conversation.id, note.id)
                    }
                  >
