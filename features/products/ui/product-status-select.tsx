...
const statusDotColors: Record<ProductStatus, string> = {
  active: '#22c55e',
  draft: '#eab308',
  archived: '#64748b',
  out_of_stock: '#ef1748'
...
export function ProductStatusSelect({
  ...
  const { updateProductStatus } = useCRMStore()
  const [pending, setPending] = React.useState(false)

  const isTable = variant === 'table'

  const handleChange = async (nextStatus: ProductStatus) => {
    if (pending) return
  ...
  return (
    ...
    >
      <Select
        ...
        disabled={pending}
        onValueChange={value => void handleChange(value as ProductStatus)}
      >
        <SelectTrigger
          className={cn(
            ...
            isTable
              ...
              : 'border-(--cf-border) bg-(--cf-element)! hover:bg-(--cf-element-hover)! text-primary [&_svg]:text-auto'
          )}
        >
