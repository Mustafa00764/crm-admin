...
export function PublicChatWidget({
  ...
  return (
    <div
      className={cn(
        'max-w-120 min-h-0 fixed top-0 left-0 max-h-full flex h-full w-full flex-col overflow-x-hidden pointer-events-auto scrollbar-thin scrollbar-thumb-rounded scrollbar-track-transparent',
        theme === 'light'
          ? 'bg-white text-slate-950 scrollbar-thumb-black/10'
          : 'bg-[#090b10] text-white scrollbar-thumb-white/20'
      )}
    >
      <div
        className={cn(
          'flex h-14 items-center justify-between border-b px-4 py-2',
        ...
        {messages.map(message => (
          ...
          >
            ...
            >
              ...
              {message.content ? (
                <div className='wrap-anywhere'>{linkifyText(message.content)}</div>
              ) : null}
            </div>
          </div>
        ))}
          ...
          >
            ...
            <div className="min-w-0 flex items-center">
              <textarea
                ...
                className={cn(
                  'min-h-9 w-full max-h-45 resize-none overflow-y-auto border-0 bg-transparent px-1 py-2 text-[13.5px] leading-5 text-slate-900 outline-none placeholder:text-black/45 placeholder:text-[13px] disabled:opacity-60 scrollbar-thin scrollbar-thumb-rounded scrollbar-track-transparent',
                  theme === 'light'
                    ? 'scrollbar-thumb-black/10'
                    : 'scrollbar-thumb-white/20'
                )}
